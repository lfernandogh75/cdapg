...
@if(isset($vehiculo) && $nfluidocontrol->activo)
         ...
         <form action="{{ route('nfluidocontrol.update','nfluido')}}" method="POST" class="formdesactivar">
          ...
          @method('PUT')
         <button type="submit" class="inline-flex justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">FINALIZAR REGISTRO DE NIVEL DE FLUIDO</button>
