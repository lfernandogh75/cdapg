...
@section('contenido')
<h2>REGISTRAR PIEZAS DE INTERIOR</h2>

<form action="/sinteriors" method="POST">
    @csrf
  <div class="mb-3">
    <label for="" class="form-label">PIEZA</label>
    <select name="pieza" class="form-select" id="pieza">
      @foreach($interiorparts as $interiorpart)
      <option value="{{$interiorpart->id}}">{{$interiorpart->name}}</option>
      @endforeach
    </select>
  </div>
  ...
  <a href="/vehiculos?placa={{$peritaje->vehiculo->placa}}&vehiculoindex=interior" class="btn btn-secondary" tabindex="5">Cancelar</a>
