...
@section('contenido')
<h2>REGISTRAR EQUIPO ELECTRICO</h2>

<form action="/selectricos" method="POST">
    @csrf
  <div class="mb-3">
    ...
    <select name="pieza" class="form-select" id="pieza" required>
      ...
      <option value="{{$electricalpart->id}}">{{$electricalpart->name}}</option>
      @endforeach
    </select>
  </div>
  <div class="mb-3">
  ...
  <a href="/vehiculos?placa={{$peritaje->vehiculo->placa}}&vehiculoindex=2" class="btn btn-secondary" tabindex="5">Cancelar</a>
