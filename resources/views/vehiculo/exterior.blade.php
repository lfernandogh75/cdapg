...
@if(isset($vehiculo) && Auth::user()->id==$responsable->id && $exteriorcontrol->activo)
         ...
         <form action="{{ route('exteriorcontrol.update','exterior')}}" method="POST" class="formdesactivar">
          @csrf
          <div class="form-group">
            <label for="exampleFormControlTextarea1">OBSERVACIÓN GENERAL</label>
            <textarea class="form-control" name="observacion" id="observacion" rows="3" required value="{{$exteriorcontrol->observacion}}" placeholder="La observación general se diligencia al final de registrar todas las piezas y se almacena al finalizar la inspección del exterior ">{{$exteriorcontrol->observacion}}</textarea>
          </div>
          <div class="form-group">
            <label for="" class="form-label">NIVEL DE APROBACIÓN</label>
            <input id="nivelaprobado" name="nivelaprobado" type="number"  placeholder="0% a 100%" step="0" min="0" max="100"   class="form-control" tabindex="3" required value="{{$exteriorcontrol->nivelaprobado}}">
          </div>
     ...
     @if(isset($exteriorcontrol->activo))
     @if($exteriorcontrol->activo==0)
     <div class="form-group">
       <label for="exampleFormControlTextarea1">OBSERVACIÓN GENERAL</label>
       <textarea class="form-control" name="observacion" id="observacion" rows="3" required value="{{$exteriorcontrol->observacion}}" placeholder="La observación general se diligencia al final de registrar todas las piezas y se almacena al finalizar la inspección del exterior " readonly>{{$exteriorcontrol->observacion}}</textarea>
     </div>
     <div class="form-group">
       <label for="" class="form-label">NIVEL DE APROBACIÓN</label>
       <input id="nivelaprobado" name="nivelaprobado" type="number"  placeholder="0% a 100%" step="0" min="0" max="100"   class="form-control" tabindex="3" value="{{$exteriorcontrol->nivelaprobado}}" required readonly>
     </div>
     @endif
  @endif
  ...
  <table id="exteriors" class="table table-dark table-striped mt-4">
  <thead>
    <tr>
  {{--    <th scope="col">ID</th>--}}
  ...
  <tbody>   
    ...
    <tr>
     {{--   <td>{{$exterior->id}}</td>--}}
        <td>{{$exterior->exteriorpart->name}}</td>
...
Swal.fire({
title: '¿finalizar el registro de exteriores ? ',
  icon: 'info',
...
this.submit();
  Swal.fire(
    'Finalizado!',
    'A finalizado el registro de exteriores .',
    'success'
  )
}
})
