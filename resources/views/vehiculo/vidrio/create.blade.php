...
@section('contenido')
<h2>REGISTRAR VIDRIOS</h2>

<form action="/svidrios" method="POST">
    @csrf
  <div class="mb-3">
    ...
    <select name="pieza" class="form-select" id="pieza" required>
      ...
      <option value="{{$vidriopart->id}}">{{$vidriopart->name}}</option>
      @endforeach
    </select>
  </div>
  ...
  <a href="/vehiculos?placa={{$peritaje->vehiculo->placa}}&vehiculoindex=vidrio" class="btn btn-secondary" tabindex="5">Cancelar</a>
